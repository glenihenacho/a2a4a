...
const bgColor = "#070B14";

// ─── PRIMITIVES ───
function Badge({ children, color, bg }) {
  return <span style={{ fontFamily: ft.mono, fontSize: 10, fontWeight: 600, color, background: bg, padding: "3px 10px", borderRadius: 100, letterSpacing: ".06em", textTransform: "uppercase", whiteSpace: "nowrap", display: "inline-block" }}>{children}</span>;
}
function VBadge({ v }) {
  const c = { SEO: { c: "#42A5F5", b: "rgba(66,165,245,.1)" }, AIO: { c: "#90CAF9", b: "rgba(144,202,249,.1)" } };
  const { c: col, b: bgc } = c[v] || c.SEO;
  return <Badge color={col} bg={bgc}>{v}</Badge>;
}
function ScoreBar({ value, color }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width: 60, height: 4, borderRadius: 2, background: "rgba(255,255,255,.05)" }}>
        <div style={{ width: `${value}%`, height: "100%", borderRadius: 2, background: color, transition: "width .5s" }} />
      </div>
      <span style={{ fontFamily: ft.mono, fontSize: 11, color: "rgba(255,255,255,.45)" }}>{value}</span>
    </div>
  );
}
function Card({ children, style: s = {}, mob }) {
  return <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(66,165,245,.07)", borderRadius: mob ? 12 : 14, padding: mob ? 16 : 24, ...s }}>{children}</div>;
}
function ScrollX({ children }) {
  return <div style={{ overflowX: "auto", WebkitOverflowScrolling: "touch", margin: "0 -4px", padding: "0 4px" }}>{children}</div>;
}

// ─── CHARTS ───
function Sparkline({ data, width = 120, height = 36, color = blue }) {
...
function BarChart({ data, labels, keys, colors, height = 180, mob }) {
  const max = Math.max(...data.map(d => keys.reduce((s, k) => s + d[k], 0)));
  const h = mob ? 140 : height;
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-around", height: h, paddingBottom: 24, position: "relative" }}>
        {[0, 0.5, 1].map(p => (
          <div key={p} style={{ position: "absolute", left: 30, right: 0, bottom: 24 + p * (h - 24), borderTop: "1px solid rgba(255,255,255,.03)", pointerEvents: "none" }}>
            <span style={{ position: "absolute", left: -30, top: -7, fontFamily: ft.mono, fontSize: 8, color: "rgba(255,255,255,.15)", width: 26, textAlign: "right" }}>${Math.round(max * p / 1000)}k</span>
          </div>
        ))}
        {data.map((d, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", zIndex: 1, flex: 1, maxWidth: 44 }}>
            <div style={{ display: "flex", flexDirection: "column-reverse", width: mob ? 20 : 30, borderRadius: "3px 3px 0 0", overflow: "hidden" }}>
              {keys.map((k, ki) => <div key={k} style={{ height: Math.max(2, (d[k] / max) * (h - 40)), background: colors[ki], transition: "height .4s" }} />)}
            </div>
            <span style={{ fontFamily: ft.mono, fontSize: mob ? 8 : 9, color: "rgba(255,255,255,.25)", marginTop: 6 }}>{labels[i]}</span>
          </div>
        ))}
      </div>
      <div style={{ display: "flex", gap: 12, marginTop: 8, flexWrap: "wrap" }}>
        {keys.map((k, i) => (
          <div key={k} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            ...
            <span style={{ fontFamily: ft.mono, fontSize: 9, color: "rgba(255,255,255,.25)" }}>{k === "clearing" ? "Clearing" : "Milestones"}</span>
          </div>
        ))}
      </div>
    </div>
...
function DonutChart({ segments, size = 100 }) {
  ...
  let cum = 0;
  const r = size * 0.36, cx = size / 2, cy = size / 2, sw = size * 0.1, circ = 2 * Math.PI * r;
  return (
    <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
      {segments.map((seg, i) => {
        const pct = seg.value / total, dashLen = circ * pct, dashOff = circ * cum;
        cum += pct;
        return <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={seg.color} strokeWidth={sw} strokeDasharray={`${dashLen} ${circ - dashLen}`} strokeDashoffset={-dashOff} strokeLinecap="round" />;
      })}
    </svg>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: mob ? "column" : "row", height: "100vh", background: bgColor, color: "#E3F2FD", fontFamily: ft.sans, overflow: "hidden", position: "relative" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}html,body{height:100%}body{background:${bgColor}}::selection{background:#1565C040;color:#fff}
        input::placeholder,textarea::placeholder{color:rgba(255,255,255,.18)}
        ::-webkit-scrollbar{width:4px;height:4px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:rgba(66,165,245,.1);border-radius:2px}
      `}</style>
